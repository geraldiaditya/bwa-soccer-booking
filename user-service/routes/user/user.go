...
func (u *UserRoute) Run() {
	...
	group.GET("/:uuid", middlewares.Authenticate(), middlewares.AuthorizeOwnerOrAdmin(), u.controller.GetUserController().GetUserByUUID)
	group.POST("/login", u.controller.GetUserController().Login)
	group.POST("/register", u.controller.GetUserController().Register)
	group.PUT("/:uuid", middlewares.Authenticate(), middlewares.AuthorizeOwnerOrAdmin(), u.controller.GetUserController().Update)
}
